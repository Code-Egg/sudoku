    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-8 px-4 font-sans">
      ...
      <header className="w-full max-w-xl flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">PokéSudoku</h1>
        </div>
      ...
      {/* Difficulty Tabs */}
      <div className="bg-white p-1 rounded-xl shadow-sm border border-slate-200 mb-8 flex gap-1 w-full max-w-md">
        {(['EASY', 'MEDIUM', 'HARD'] as Difficulty[]).map((level) => (
          <button
            ...
            `}
          >
            {level === 'EASY' ? '3x3 (Easy)' : level === 'MEDIUM' ? '6x6 (Med)' : '9x9 (Hard)'}
          </button>
        ))}
      </div>
      ...
      <main className="w-full max-w-xl flex flex-col items-center">
        ...
        <Grid 
          ...
          onCellClick={(idx) => !isGameComplete && setCells(prev => {
             // Only allow selection if not fixed (optional, but standard UX allows selecting fixed cells just not editing them)
             // Let's allow selecting any cell for visibility.
        ...
        <div className="mt-8 flex w-full max-w-md gap-4">
          ...
          {/* Validate Button - Optional, since we auto-check on fill, but good for "giving up" or checking progress if we didn't auto-check */}
        </div>
